<script lang="ts">
	...
	type Project = { id: string; name: string; instructions: string; ai_behaviour: string; items?: unknown[] };
	...
	function resetForm() { editingId = null; form = { name: '', instructions: '', aiBehaviour: '' }; }
	...
	function edit(project: Project) {
		...
		form = { name: project.name, instructions: project.instructions || '', aiBehaviour: project.ai_behaviour || '' };
	}
	...
	async function saveProject() {
		...
		try {
			...
			else await api.post(`/mcp/workspaces/${workspaceId}/projects`, form);
			resetForm();
			await loadProjects();
	...
	{:else}
		<Card><CardHeader><CardTitle>{editingId ? 'Edit project' : 'Create project'}</CardTitle></CardHeader><CardContent class="space-y-4">
			...
			<div class="flex flex-wrap gap-2">
				<Button onclick={saveProject} disabled={saving || !form.name.trim()}>{#if saving}<LoaderCircle class="size-4 animate-spin" />{:else if editingId}<Save class="size-4" />{:else}<Plus class="size-4" />{/if}{editingId ? 'Save changes' : 'Create project'}</Button>
				{#if editingId}<Button variant="outline" onclick={resetForm}><X class="size-4" />Cancel edit</Button>{/if}
			</div>
		</CardContent></Card>
				...
				{#each projects as project}
					<div class="rounded-md border p-3 {editingId === project.id ? 'border-primary/70 bg-primary/5' : ''}">
						<div class="flex items-start justify-between gap-3">
							...
							<div class="flex shrink-0 gap-1"><Button variant="ghost" size="icon" aria-label="Edit project" onclick={() => edit(project)}><Pencil class="size-4" /></Button><Button variant="ghost" size="icon" aria-label="Delete project" onclick={() => remove(project.id)}><Trash2 class="size-4" /></Button></div>
						</div>
					</div>
				{/each}
			</div>{/if}
