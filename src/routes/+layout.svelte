<script lang="ts">
	...
	async function redirectIfLicenceInvalid(refresh = false) {
		if (!auth.isAuthenticated || !auth.isAdmin || page.url.pathname.startsWith('/admin/license')) return;
		try {
			const summary = await api.get<{ enforcement?: boolean; reason?: string; components?: Record<string, { allowed?: boolean; reason?: string | null }> }>(`/license/status${refresh ? '?refresh=1' : ''}`);
			const panel = summary.components?.orbitfs_panel;
			if (summary.enforcement !== false && panel?.allowed !== true) {
				goto(`/admin/license?reason=${encodeURIComponent(panel?.reason ?? summary.reason ?? 'panel_invalid')}`);
			}
		} catch {
